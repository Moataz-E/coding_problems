...
def solution(N: int) -> int:
    """Calculate binary gap of an integer.

    Codility Score: 100%.
    Time Complexity: O(n) where n is number of digits of N in binary form.
    """
    bin_str = "{0:b}".format(N)
    segments = bin_str.split('1')
    segments_fltr = list(filter(None, segments))
    if not segments_fltr: return 0
    if (bin_str[-1] == '0'): segments_fltr.pop()
    if not segments_fltr: return 0
    if (bin_str[0] == '0'):
        return max(len(x) for x in segments_fltr[1:])
    else:
        return max(len(x) for x in segments_fltr)
